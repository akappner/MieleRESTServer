use cbc;
//use aes::cipher::{block_padding::Pkcs7, BlockDecryptMut, BlockEncryptMut, KeyIvInit, generic_array::GenericArray,};
use aes::{cipher::{BlockDecryptMut, KeyIvInit, BlockEncryptMut}, Aes256};
use rand::prelude::*;
use std::fmt;
use aes::cipher::{block_padding::NoPadding,
    BlockCipher, BlockEncrypt, BlockDecrypt, KeyInit,
};

struct GroupId([u8; 8]);

impl GroupId {
    fn random () -> Self
    {
        let mut rng = rand::rng();
        return GroupId { 0: rng.random()}
    }
    fn from_hex (s: String)->Self
    {
        GroupId {0: hex::decode(s).unwrap().try_into().unwrap()}
    }
}

impl fmt::Display for GroupId {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        for byte in &self.0 {
            write!(f, "{:02x}", byte)?;
        }
        Ok(())
    }
}
struct AesKey([u8; 32]);

struct MieleHmac ([u8; 16]);

struct AesIv ([u8; 16]);
impl AesIv {
    fn random () -> Self
    {
        let mut rng = rand::rng();
        return AesIv { 0: rng.random()}
    }
}
struct MieleSignature 
{
    iv : AesIv,
    hmac : MieleHmac,
}
impl MieleSignature {
    fn from_hex (s: String)->MieleSignature
    {
        let b = hex::decode(s).unwrap();
        println!("b length {}", b.len());
        MieleSignature { iv: AesIv{0: b[..16].try_into().unwrap()}, hmac: MieleHmac{0: b[16..32].try_into().unwrap()} } 
    }
}
impl fmt::Display for AesKey {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        for byte in &self.0 {
            write!(f, "{:02x}", byte)?;
        }
        Ok(())
    }
}

impl AesKey {
    fn random () -> Self
    {
        let mut rng = rand::rng();
        return AesKey { 0: rng.random()}
    }
}

struct MieleCryptoContext
{
    group_id : GroupId,
    group_key : AesKey,
}

type encryptor = cbc::Encryptor<aes::Aes256>;
type decryptor = cbc::Decryptor<aes::Aes256>;

impl MieleCryptoContext 
{
    fn default() -> Self
    {
        return MieleCryptoContext {group_id: GroupId{0: [0x31; 8]}, group_key: AesKey{0: [0x31; 32]}};
    }
    fn random () -> Self
    {
        let mut rng = rand::rng();
        return MieleCryptoContext {group_id: GroupId::random(), group_key: AesKey::random()};
    }
    fn decrypt (&self, buffer : Vec<u8>, iv : &AesIv)-> Vec<u8>
    {
        println!("buffer len: {:?}", buffer.len());
        let mut d: cbc::Decryptor<Aes256> = decryptor::new_from_slices(&self.group_key.0, &iv.0).unwrap();
        return d.decrypt_padded_vec_mut::<NoPadding>(buffer.as_slice())
.unwrap()    }
    fn encrypt (&self, buffer : Vec<u8>, iv : &AesIv)->Vec<u8>
    {
        let mut e: cbc::Encryptor<Aes256> = encryptor::new_from_slices(&self.group_key.0, &iv.0).unwrap();
        return e.encrypt_padded_vec_mut::<NoPadding>(buffer.as_slice());
    }
}
impl std::fmt::Display for MieleCryptoContext {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "({}:{})", self.group_id, self.group_key)
    }
}

struct MieleHeader{
    group_id : GroupId,
    signature : MieleSignature,
}
impl MieleHeader
{
    fn to_http_header(&self) -> String
    {
        return "X-Signature: MieleH256 1111111111111111:DD361380F1AB6BC95C3A42144DA458CB58A204A4A509E14B59B7690D1846AAE3".to_string();
    }
    fn from_http_header(s: String) -> Self
    {
        return MieleHeader {group_id: GroupId::from_hex("1111111111111111".to_string()), signature: MieleSignature::from_hex("DD361380F1AB6BC95C3A42144DA458CB58A204A4A509E14B59B7690D1846AAE3".to_string()), }
    }
}
mod tests {
    use super::*;
    #[test]
    fn test_round_trip() {
       let plaintextString = "0123456789012345678901234567890123456789012345678901234567891234";
       let context : MieleCryptoContext = MieleCryptoContext::random(); 
       let iv = AesIv::random();
       let ciphertext = context.encrypt(plaintextString.as_bytes().into(), &iv);
       let plaintext = context.decrypt(ciphertext, &iv);
       //println!("{:}", context);
       assert_eq!(plaintextString.as_bytes(), plaintext);
       
    }
    #[test]
    fn test_decryption()
    {
        let header = MieleHeader::from_http_header("".to_string());
        let context = MieleCryptoContext::default();
        
        let ciphertext = hex::decode("").unwrap();
        let plaintext = context.decrypt(ciphertext, &header.signature.iv);
        println!("{:?}", plaintext);
        println!("{:?}", str::from_utf8(&plaintext).unwrap());
        assert_eq!(&plaintext, "fadsfa".to_string().as_bytes());
    }
}